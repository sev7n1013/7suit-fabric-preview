...
//       -> 布料圖依比例平鋪、裁進「外套專屬」遮罩形狀、疊皺褶陰影 -> 蓋在底圖的外套部分上面
//       襯衫部分因為遮罩本來就排除它，永遠不會被布料蓋到，也就不會有拼接縫或缺角問題

const CANVAS_W = 896;
const CANVAS_H = 1200;
const DEFAULT_SCALE = 0.22; // 布料紋理預設大小（相對畫布寬度的比例）
...
// 最多保留 4 筆比較紀錄： { fabricImg, scale, dataUrl, img }
const history = [];
let activeIndex = -1;
...
// 把布料圖片依「比例」重複平鋪畫滿指定尺寸的 canvas（像磁磚一樣重複貼，比例可調整）
function drawTiledPattern(targetCtx, img, w, h, scale) {
...
// 核心合成：底圖（西裝+襯衫）-> 布料(依比例平鋪，只裁進外套遮罩) -> 疊皺褶陰影 -> 蓋在外套上
function renderComposite(fabricImg, scale) {
  if (!alphaMaskCanvas) {
    alphaMaskCanvas = maskToAlphaCanvas(suitMaskImg);
  }

  // Step 1: 布料依比例平鋪一張暫存 canvas
  const fabricCanvas = document.createElement('canvas');
  fabricCanvas.width = CANVAS_W;
  fabricCanvas.height = CANVAS_H;
  const fctx = fabricCanvas.getContext('2d');
  drawTiledPattern(fctx, fabricImg, CANVAS_W, CANVAS_H, scale);

  // Step 2: 用 alpha 遮罩把布料裁進「外套專屬」形狀（destination-in），襯衫範圍不受影響
  fctx.globalCompositeOperation = 'destination-in';
  fctx.drawImage(alphaMaskCanvas, 0, 0);
  fctx.globalCompositeOperation = 'source-over';

  // Step 3: 疊上灰階皺褶/打光層，讓布料貼合西裝立體感
...
// 切換到某筆歷史紀錄：套用該筆自己記錄的比例，並同步滑桿位置
function selectHistory(i) {
  const item = history[i];
  currentScale = item.scale;
  scaleSlider.value = String(item.scale);
  renderComposite(item.fabricImg, item.scale);
  emptyHint.style.display = 'none';
  retakeBtn.disabled = false;
  downloadBtn.disabled = false;
...
cameraInput.addEventListener('change', async (e) => {
  ...
  try {
    ...
    renderComposite(fabricImg, currentScale);
    const dataUrl = resultCanvas.toDataURL('image/png');
    const resultImg = await loadImage(dataUrl);

    if (history.length >= 4) history.shift();
    history.push({ fabricImg, scale: currentScale, dataUrl, img: resultImg });
    activeIndex = history.length - 1;
    renderThumbs();
    scaleControl.style.display = 'flex';
    showToast('布料套用完成 ✅ 覺得紋理太大/太小可以拉下面的滑桿調整');
  } catch (err) {
    console.error(err);
...
scaleSlider.addEventListener('input', (e) => {
  ...
  if (activeIndex < 0) return;
  renderComposite(history[activeIndex].fabricImg, currentScale);
});
